...
async function run() {
  ...
  try {
    ...
    assert.match(await page.locator('#market-results').innerText(), /검색 조건에 맞는 제품이 없습니다/);
    ...
    const values = (await page.locator('tr[data-product] td:nth-child(3)').allTextContents()).map(s => Number(s.replace(/[^\d]/g, '')));
    ...
    assert.match(await page.locator('.simulation-result').innerText(), /4,000원/);
    await page.keyboard.press('Escape');

    await page.locator('[data-page="data"]').click();
    await page.locator('[data-action="upload"]').first().click();
    await page.locator('#upload-file').setInputFiles({ name: 'invalid.json', mimeType: 'application/json', buffer: Buffer.from('[{"brand":"missing"}]') });
    await page.waitForSelector('.form-error');
    assert.equal(await page.locator('#apply-import').isDisabled(), true);
    const fixture = JSON.parse(fs.readFileSync(path.join(__dirname, '../data/extracted/세탁세제.json'), 'utf8'))[0];
    fixture.product_name = '<img src=x onerror=alert(1)> 테스트제품';
    fixture.product_url = 'javascript:alert(1)';
    await page.locator('#upload-file').setInputFiles({ name: 'sample.json', mimeType: 'application/json', buffer: Buffer.from(JSON.stringify([fixture])) });
    ...
    assert.ok((await page.locator('dialog a[target="_blank"]').getAttribute('href')).startsWith('https://www.coupang.com/np/search'));
    await page.keyboard.press('Escape');
    ...
    assert.equal((await docEvent).suggestedFilename(), '신제품_생산견적요청서.docx');
    await page.locator('[data-page="usp"]').click();
    assert.match(await page.locator('.empty-state').innerText(), /아직 수집된 데이터가 없어/);
    ...
    for (const url of ['/.git/config', '/dashboard/server.py', '/api/unknown']) {
      assert.equal((await page.request.get(base + url)).status(), 404);
    }
    assert.equal((await page.request.post(base + '/api/import', { data: { overrides: { bad: [] } } })).status(), 400);
    assert.equal((await page.request.post(base + '/api/simulate', { data: { index: 0, price: -1 } })).status(), 400);
    ...
    console.log('PASS: real data, search, filters, pagination, sorting, details, simulation, invalid/valid imports, XSS escaping, restoration, downloads, USP guide, API safety, and mobile navigation/overflow.');
  } finally {
    await browser.close();
  }
}
