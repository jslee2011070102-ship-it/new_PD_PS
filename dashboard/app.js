...
const paths = {
  ...
  sparkles:
    '<path d="m12 3 2.7 6.3L21 12l-6.3 2.7L12 21l-2.7-6.3L3 12l6.3-2.7L12 3ZM20 2v4m-2-2h4M3 19v3m-1.5-1.5h3"/>',
  ...
  github:
    '<path d="M9 19c-5 1-5-3-7-3m14 6v-4c0-1-.4-2-1-2 4 0 6-2 6-5 0-2-1-3-1-3s0-3-1-4c-2 0-3 1-3 1-3-1-5-1-8 0 0 0-1-1-3-1-1 1-1 4-1 4s-1 1-1 3c0 3 2 5 6 5-.6 0-1 1-1 2v4"/>',
  ...
  capsule:
    '<rect x="4" y="6" width="16" height="12" rx="6" transform="rotate(-40 12 12)"/><path d="m8 7 8 10"/>',
  spray:
    '<path d="M10 7V4h7l3 2M10 4H6V2h10v2M10 7l-4 5v9h12v-9l-4-5h-4ZM6 15h12"/>',
...
function roleBadge(role) {
  return `<span class="pill ${role === "본품" ? "green" : role === "리필" ? "blue" : ""}">${esc(role)}</span>`;
}
function toast(text) {
...
function overview() {
  ...
  const cards = [
    [
      ...
      "개",
      "layers",
      '<span class="green-dot"></span><b>생활용품</b> 핵심 카테고리',
    ],
    [
      "분석 제품",
      s.count,
      "개",
      "box",
      `<span class="green-dot"></span><b>${s.brands}개 브랜드</b> 경쟁 제품 수집`,
    ],
    [
      "신제품 목표 규격",
      specs.length,
    ...
    [
      ...
      "%",
      "shield",
      `${icon("check")}<b>${s.validation["일치"] || 0}건 일치</b> · ${s.validation["표기없음"] || 0}건 표기없음`,
    ],
  ];
  const ordered = cats
    .flatMap((cat) => specs.filter((p) => p.cat === cat && p.role === "본품"))
    .slice(0, 4);
  return `${pageHeading("리서치 대시보드", "시장의 흐름을 읽고, 다음 제품의 가능성을 발견하세요.")}${sessionBanner()}
    ...
    <section class="workflow"><div class="panel-heading"><h2>리서치 진행 현황</h2><span class="pill green">1차 조사</span></div><div class="workflow-list">${[
      ["썸네일 수집", "완료"],
      ["가격 · 시장 데이터 분석", "완료"],
      ["신제품 목표 규격 도출", "완료"],
      ["상세페이지 USP 분석", "수집 대기"],
      ...
      .map(
        ([name, status], i) =>
          `<a href="#${["data", "market", "specs", "usp", "documents"][i]}" class="workflow-step ${i === 3 ? "waiting" : ""}"><span class="step-dot">${i === 3 ? "4" : icon("check")}</span><span>${name}</span><span>${status}</span></a>`,
      )
...
function filteredProducts() {
  ...
  const products = state.data.products.filter(
    (p) =>
      ...
      (!state.role || p.product_role === state.role) &&
      (!state.basis || p.basis === state.basis) &&
      (!state.validation ||
        (state.validation === "확인 필요"
          ? !["일치", "표기없음"].includes(p.validation)
          : p.validation === state.validation)) &&
      `${p.brand || ""} ${p.product_name || ""}`.toLowerCase().includes(query),
  );
  return products.sort((a, b) =>
    state.sort === "price"
      ? (a.price_krw ?? Infinity) - (b.price_krw ?? Infinity)
      : state.sort === "revenue"
...
function uspPage() {
  return `${pageHeading("USP 분석", "가격 너머의 이유, 경쟁 제품이 고객에게 전달하는 핵심 가치를 살펴보세요.", false)}<section class="panel"><div class="panel-heading"><h2>상세페이지 리서치</h2><span class="pill amber">데이터 수집 대기</span></div><div class="empty-state"><div class="empty-icon">${icon("sparkles")}</div><h2>좋은 제품에는, 선택받는 이유가 있습니다.</h2><p>상세페이지 USP 분석 도구는 준비되어 있습니다.<br>목표 제품의 캡처를 수집한 후, 핵심 소구점과 근거 문구를 분석해 보세요.<br>아직 수집된 데이터가 없어 분석 결과는 표시하지 않습니다.</p><button class="button primary" data-action="usp-guide">${icon("book")}상세페이지 수집 가이드</button> <a href="#market" class="button">분석할 제품 찾기 ${icon("arrow-right")}</a></div></section><div class="steps-grid">${[
    [
      "01",
      "목표 제품 선정",
      "전체 150개가 아닌, 가격 분석에서 선정한 목표 제품을 우선 조사하세요.",
    ],
    [
      "02",
      "상세페이지 캡처",
      "상세페이지를 맨 아래까지 스크롤한 후 인쇄 → PDF로 저장하세요.",
    ],
    [
      "03",
      "USP 추출 · 검증",
      "성분, 인증, 기능 등 소구점과 원문 근거를 추출하고 기존 Python 도구로 검증하세요.",
    ],
  ]
    .map(
      ([n, t, p]) =>
        `<article class="step-card"><span class="step-number">STEP ${n}</span><h3>${t}</h3><p>${p}</p></article>`,
    )
    .join("")}</div>`;
}
function documentsPage() {
  return `${pageHeading("산출물 관리", "리서치 결과를 팀과 공유하고, 생산 협의를 위한 다음 단계로 연결하세요.", false)}${sessionBanner()}<div class="resource-grid">${[
    [
      "file",
      "신제품 생산 견적요청서",
      "6개 카테고리의 본품·리필 12종 목표 규격과 선정 근거를 담은 공장 전달용 문서입니다.",
      "DOCX",
      "12개 목표 규격",
      "/api/document",
      "견적요청서 다운로드",
    ...
    [
      "grid",
      "시장 조사 데이터",
      "현재 워크스페이스의 제품, 가격, 단가 검증, 추정 매출을 엑셀로 내보냅니다.",
      "XLSX",
      `${state.data.products.length}개 제품`,
      "export",
      "엑셀 다운로드",
    ],
    [
      "database",
      "신제품 목표 규격 원본",
      "목표 판매가, 생산원가, 벤치마크와 경쟁사 근거 데이터가 담긴 구조화된 원본입니다.",
      "JSON",
      "12개 목표 규격",
      "/api/specs.json",
      "원본 데이터 다운로드",
    ...
    .map(
      ([i, t, p, type, meta, url, label]) =>
        `<article class="resource-card"><div class="file-icon">${icon(i)}</div><h2>${t}</h2><p>${p}</p><div class="resource-meta"><span class="pill green">${type}</span><span class="pill">${meta}</span></div>${url === "export" ? `<button class="button" data-action="export">${icon("download")}${label}</button>` : `<a class="button" href="${url}" download>${icon("download")}${label}</a>`}</article>`,
    )
    .join(
      "",
...
function showDialog(title, subtitle, body, footer = "") {
  $("#dialog-content").innerHTML =
    `<div class="dialog-head"><div><h2 id="dialog-title">${title}</h2><p>${subtitle}</p></div><button class="icon-button" data-action="close" aria-label="닫기">${icon("close")}</button></div><div class="dialog-body">${body}</div>${footer ? `<div class="dialog-footer">${footer}</div>` : ""}`;
...
function productDetail(id) {
  ...
  showDialog(
    "경쟁 제품 상세",
    `${esc(p.category)} · 카테고리 ${number(p.rank)}위 · ${esc(p.row["조사일자"])}`,
    `<div class="flex-actions">${roleBadge(p.product_role)}<span class="pill">${esc(p.product_form)}</span>${badge(p.validation)}</div><h3 class="detail-title">${esc(p.product_name || "제품명 미확인")}</h3><div class="detail-grid">${[
      ["판매가", won(p.price_krw)],
      ["단위당 가격", `${won(p.unit_price)} / ${esc(p.basis || "미확인")}`],
      ["용량 · 구성", `${esc(p.capacity_text)} × ${esc(p.composition_text)}`],
      ["리뷰 수", number(p.review_count)],
      [
        "월 구매자수 (하한)",
        p.buyers == null ? "확인 불가" : `${number(p.buyers)}명 이상`,
      ],
      [
        "추정 월매출",
        p.revenue == null ? "계산 제외" : `${revenue(p.revenue)}원`,
      ],
    ]
      .map(([k, v]) => `<div><small>${k}</small><strong>${v}</strong></div>`)
      .join(
        "",
      )}</div><div class="detail-section"><h3>검증 근거</h3><p>쿠팡 표기: ${esc(p.unit_price_text || "표기 없음")}<br>구매자수 원문: ${esc(p.monthly_buyers_text || "확인 불가")}<br>형태 판단: ${esc(p.form_reason || "별도 근거 없음")}</p></div>${p.notes ? `<div class="detail-section"><h3>수집 메모</h3><p>${esc(p.notes)}</p></div>` : ""}<div class="form-hint">월매출은 월구매자수 × 판매가 × 2로 계산한 비교용 추정치입니다. 실제 상품 URL을 모르는 경우 제품명 검색 링크를 제공합니다.</div>`,
    `<button class="button" data-action="close">닫기</button>${actualURL || searchURL ? `<a class="button primary" href="${esc(actualURL || searchURL)}" target="_blank" rel="noopener noreferrer">${actualURL ? "상품 페이지" : "쿠팡에서 제품명 검색"} ${icon("external")}</a>` : ""}`,
  );
}
...
function guide(usp = false) {
  const entries = usp
    ? [
        [
          "목표 제품만 선택하세요",
          "시장 분석에서 같은 단가기준의 경쟁 제품을 확인하세요. 전수조사가 아니라 목표 제품 위주로 조사하는 것이 효율적입니다.",
        ],
        [
          "상세페이지를 끝까지 스크롤하세요",
          "지연 로딩 이미지가 모두 표시되면 Ctrl+P → PDF로 저장합니다. 1~2쪽짜리 PDF라면 이미지 누락을 확인하세요.",
        ],
        [
    ...
    : [
        [
          "시장 분석에서 근거를 확인하세요",
          "제품명·브랜드 검색, 카테고리·역할·단가 기준·검증 상태 필터를 사용할 수 있습니다. 행을 누르면 원문과 계산 근거를 확인할 수 있습니다.",
        ],
        [
          "신제품 기획에서 원가를 검토하세요",
        ...
        [
          "산출물을 다운로드하세요",
          "현재 제품 데이터는 XLSX로, 원본 기획안은 DOCX·JSON으로 받을 수 있습니다. 원본 기획안은 업로드와 별개입니다.",
        ],
      ];
  showDialog(
    usp ? "상세페이지 수집 가이드" : "Product Lab 이용 가이드",
    "이미지를 읽는 일과 숫자를 계산하는 일을 분리합니다.",
    `<div class="guide-list">${entries.map(([title, desc], i) => `<div><span>0${i + 1}</span><section><h3>${title}</h3><p>${desc}</p></section></div>`).join("")}</div>${usp ? '<div class="detail-section"><h3>기존 도구 실행 예시</h3><code class="mono">python thumbnail_analyzer/analyze_details.py build<br>--input usp.json --category 세탁세제<br>--output 결과/세탁세제_USP.xlsx</code></div>' : ""}`,
...
function openUpload() {
  ...
  zone.addEventListener("drop", (e) => {
    ...
    validateUpload(e.dataTransfer.files[0]);
  });
}
...
async function validateUpload(file) {
  ...
  try {
    if (!file.name.toLowerCase().endsWith(".json"))
      throw new Error(
        "JSON 파일만 불러올 수 있습니다. 이미지/PDF는 먼저 추출 JSON으로 변환해 주세요.",
      );
    if (file.size > 3 * 1024 * 1024)
      throw new Error("3MB 이하의 JSON 파일을 선택하세요.");
    ...
    if (sequence === uploadSequence && $("#upload-result"))
      $("#upload-result").innerHTML =
        `<div class="form-error">${esc(error.message)}</div>`;
  }
}
async function exportExcel(button) {
  button.disabled = true;
  try {
    const response = await api("/api/export.xlsx", {
    ...
    link.href = url;
    link.download = "market_research.xlsx";
    document.body.append(link);
    link.click();
    link.remove();
    setTimeout(() => URL.revokeObjectURL(url), 10000);
    toast(
      `${state.data.products.length}개 제품의 엑셀 파일을 다운로드했습니다.`,
    );
  } catch (error) {
    toast(error.message);
  } finally {
...
function notifications() {
  ...
  showDialog(
    "리서치 알림",
    "현재 데이터에서 확인해야 할 항목입니다.",
    `<div class="guide-list"><div><span>${icon("clock")}</span><section><h3>USP 상세페이지 수집 대기</h3><p>분석 도구는 준비되어 있지만 상세페이지 원본과 추출 데이터는 아직 없습니다.</p></section></div><div><span>${icon("shield")}</span><section><h3>단가 표기 없음 · ${s.validation["표기없음"] || 0}건</h3><p>가격 표기가 없거나 캡처에 가려져 검산할 수 없는 항목입니다. 오류로 분류하지 않습니다.</p></section></div><div><span>${icon("file")}</span><section><h3>원본 견적요청서 준비 완료</h3><p>6개 카테고리 12개 목표 규격의 공장 전달용 DOCX를 다운로드할 수 있습니다.</p></section></div></div>`,
    '<button class="button" data-action="show-unverified">표기없는 제품 확인</button><button class="button primary" data-action="close">확인</button>',
  );
}
...
document.addEventListener("click", (e) => {
  const el = e.target.closest(
    "[data-action], [data-product], [data-spec], [data-category], [data-category-go], [data-spec-category], [data-pagination]",
  );
  ...
  if (el.dataset.categoryGo)
    return navigate("market", { category: el.dataset.categoryGo });
  ...
  if (el.dataset.specCategory) {
    state.specCategory = el.dataset.specCategory;
    render();
    return;
  }
  if (el.dataset.pagination) {
    state.pageNumber = Number(el.dataset.pagination);
    renderProducts();
    return;
  }
  switch (el.dataset.action) {
    ...
    case "workspace":
      showDialog(
        "생활용품 리서치 워크스페이스",
        "Product Lab · 쿠팡 1차 시장 조사",
        '<div class="notice-box">이 대시보드는 저장소에 커밋된 실제 제품 데이터를 사용합니다.<br>조사일: 2026.09.21 · 6개 카테고리 · 최초 150개 제품<br>개인 계정, 로그인 및 공동 편집 기능은 연결되어 있지 않습니다.</div>',
        '<button class="button primary" data-action="close">확인</button>',
      );
      break;
    case "search":
      navigate("market");
    ...
    case "apply-import":
      if (state.pendingImport) {
        state.data = state.pendingImport.result;
        state.overrides = state.pendingImport.overrides;
        state.pendingImport = null;
        state.pageNumber = 1;
        dialog.close();
        render();
        toast("검증된 데이터를 현재 탭에 적용했습니다.");
      }
      break;
    case "reset-data":
...
document.addEventListener("change", (e) => {
  ...
  if (e.target.id === "role-category") {
    const category = state.data.summary.categories.find(
      (c) => c.name === e.target.value,
    );
    $("#role-chart").innerHTML = roleChart(
      category ? category.roles : state.data.summary.roles,
    );
  }
  ...
  try {
    const result = await (
      await api("/api/simulate", {
        ...
        price: Number($("#simulation-price").value),
      })
    ).json();
    if ($("#simulation-output"))
      $("#simulation-output").innerHTML =
        `<div class="simulation-result"><div><small>묶음 목표 생산원가</small><strong>${won(result.cost)}</strong></div><div><small>낱개 환산 원가</small><strong>${won(result.costEach)}</strong></div><div><small>기준 제품 단가 대비</small><strong>${result.benchDiff > 0 ? "+" : ""}${number(result.benchDiff)}%</strong></div></div>`;
  } catch (error) {
    if ($("#simulation-output"))
      $("#simulation-output").innerHTML =
        `<div class="form-error">${esc(error.message)}</div>`;
...
async function load() {
  try {
    state.data = await (await api("/api/dashboard")).json();
    state.original = state.data;
    render();
  } catch (error) {
    main.innerHTML = `<div class="empty-state"><div class="empty-icon">${icon("database")}</div><h2>데이터를 불러오지 못했습니다</h2><p>${esc(error.message)}</p><button class="button primary" data-action="retry">다시 시도</button></div>`;
  }
}
hydrateIcons();
load();
